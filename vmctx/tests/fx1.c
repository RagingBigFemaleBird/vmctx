...
static void one(int round, const char *name, char *const argv[])
{
	...
	pid_t waited;
	do { waited = waitpid(kid, &st, 0); } while (waited < 0 && errno == EINTR);
	if (waited != kid) {
...
static void *background_noise(void *arg)
{
	...
	(void)arg;
	while (atomic_load_explicit(&keep_spinning, memory_order_relaxed))
		x++;
	return NULL;
}
...
int main(void)
{
	...
	int error = pthread_create(&noise, NULL, background_noise, NULL);
	if (error) {
		fprintf(stderr, "FAIL fx1: noise thread: %s\n", strerror(error));
		return 2;
	}
	for (round = 0; round < ROUNDS; round++) {
		error = pthread_create(&worker, NULL, spawn_from_thread,
			       (void *)(long)(100 + round));
		if (error) {
			fprintf(stderr, "fx1: worker thread: %s\n", strerror(error));
			fails++;
			break;
		}
		error = pthread_join(worker, NULL);
		if (error) {
			fprintf(stderr, "FAIL fx1: worker join: %s\n", strerror(error));
			return 2;
		}
	}
	atomic_store_explicit(&keep_spinning, 0, memory_order_relaxed);
	error = pthread_join(noise, NULL);
	if (error) {
		fprintf(stderr, "FAIL fx1: noise join: %s\n", strerror(error));
		return 2;
	}
