...
int main(void)
{
	...
	if (prctl(PR_SET_VMA, PR_SET_VMA_ANON_NAME, (unsigned long)page, PGSZ,
	          (unsigned long)"vmctx-an1")) {
		perror("an1: cannot name anonymous mapping");
		return 1;
	}
	madvise(page, PGSZ, MADV_NOHUGEPAGE);
	named = is_named(page);
	if (!named) { puts("FAIL: named anonymous mapping was not verified"); return 1; }
	...
	*stamp = 0;
	...
	for (i = 0; i < ROUNDS; i++) {
		...
		if (write(pfd[1], page, sizeof(got)) != (ssize_t)sizeof(got) ||
			...
			if (pthread_join(t, NULL)) _exit(2);
			return 1;
		}
		if (got < last) {
			printf("FAIL: named anonymous counter regressed at round %d: %lu < %lu\n", i, got, last);
			atomic_store_explicit(&stop, 1, memory_order_release);
			if (pthread_join(t, NULL)) _exit(2);
			return 1;
		}
	...
	atomic_store_explicit(&stop, 1, memory_order_release);
	if (pthread_join(t, NULL)) { puts("FAIL: stamper join failed"); return 1; }
	stamper_top = *stamp;
	...
	printf("an1: owner saw the counter reach %lu while the stamper reached "
	       "%lu; longest frozen run %d of %d (NAMED=%s; completed=%d)\n",
	       top, stamper_top, worst_frozen, ROUNDS, named ? "yes" : "no", i);
