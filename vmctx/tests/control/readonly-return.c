...
/* A source read of a remotely owned RO page must recall exclusive custody
 * before the executor can subsequently upgrade it and write new bytes. */
#define _GNU_SOURCE
#include <stdio.h>
#include <string.h>
#include <sys/mman.h>
...
int main(void)
{
    ...
    int fd=memfd_create("readonly-return",MFD_CLOEXEC),channel[2];
    ...
    for(unsigned i=0;i<sizeof(seed);i++)if(page[i]!=seed[i]) {
        fprintf(stderr,"FAIL: initial read at %u\n",i);return 1;
    }
    if(write(channel[1],(const void *)page,4096)!=4096 ||
       read(channel[0],observed,4096)!=4096 || memcmp(seed,observed,4096)) {
        fputs("FAIL: source read of read-only page\n",stderr);return 1;
    }
    ...
    for(unsigned i=0;i<sizeof(seed);i++)if(observed[i]!=(unsigned char)(seed[i]^0xa5)) {
        fprintf(stderr,"FAIL: read-only return then upgrade at %u got %02x expected %02x (initial %02x, executor %02x)\n",
    ...
    puts("PASS: read-only source recall then upgrade preserves current bytes and private file isolation");
    return 0;
}
