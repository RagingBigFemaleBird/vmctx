...
static _Atomic(volatile unsigned *) sibling_word;
...
static int child_check(const volatile unsigned *canary, int depth)
{
	...
	volatile unsigned *word = atomic_load(&sibling_word);
	if (!word || *word != CANARY)
...
int main(void)
{
	...
	memset(&f, 0, sizeof(f));
	if (pthread_barrier_init(&ready, NULL, NTHREADS + 1))
		return 2;
	...
	/* The pointer's stack frame must exist before the first fork. */
	pthread_barrier_wait(&ready);
