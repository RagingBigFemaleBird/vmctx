 ...
 * creates the loan. A pipe write consumes the buffer: the call is forwarded, and
...
int main(void)
{
	...
	static unsigned long received[NW];
	/*
	...
	/* /dev/null returns the count without reading the buffer. One page fits
	 * an empty Linux pipe; nonblocking mode makes a broken transfer an error
	 * instead of leaving this control stuck in read/write. */
	if (pipe2(pipefd, O_CLOEXEC | O_NONBLOCK)) {
		printf("lo1: pipe failed\nFAIL\n");
		return 1;
	}
	...
	if (pthread_join(t, NULL)) {
		printf("lo1: pthread_join failed\nFAIL\n");
		return 1;
	}
	/* The thread is gone, and with it the shadow that holds the loan. */
	if (borrow_failed) {
		printf("lo1: the worker did not transfer the page\nFAIL\n");
		return 1;
	}
	if (read(pipefd[0], received, sizeof(received)) != sizeof(received)) {
		printf("lo1: short pipe read\nFAIL\n");
		return 1;
	}
	close(pipefd[0]);
	close(pipefd[1]);
	for (i = 0; i < NW; i++) {
		if (received[i] != pat(i)) {
			printf("lo1: source read wrong bytes at word %u\nFAIL\n", i);
			return 1;
		}
	}
