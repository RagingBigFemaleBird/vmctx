...
struct side { int rd, wr; unsigned long behind, ahead, shortread, done; const char *who;
	      const void *buf, *got; };
...
static void *other(void *p)
{
	...
	s->done = roundtrip(s->rd, s->wr, ROUNDS, &s->behind, &s->ahead, &s->shortread,
		  s->who, &s->buf, &s->got);
	return NULL;
}
...
int main(void)
{
	...
	if (pthread_join(t, NULL)) {
		puts("FAIL: pg3 worker could not be joined");
		return 1;
	}
	...
	int passed = done == ROUNDS && b.done == ROUNDS &&
		(a.behind + a.ahead + a.shortread + b.behind + b.ahead + b.shortread) == 0;
	if (!passed) printf("pg3: completed A %lu B %lu; required %d each\n", done, b.done, ROUNDS);
