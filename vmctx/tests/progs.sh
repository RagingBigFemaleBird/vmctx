...
set -u
HERE=${VMCTX_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}
...
run() {
	...
	local d="$OUTD/$name"
	local LIMIT=$LIMIT
	# CPIO transfers this complete archive in 512-byte calls. Keep its full
	# roundtrip and allow the measured execution time; explicit limits win.
	[ "$name" != cpiort ] || LIMIT=${VMR_LIMIT:-180}
...
verdict() {	# <case> <ok:0/1> <why>
	...
	# Output markers cannot make a failed workload pass. timeout1 explicitly
	# tests timeout's exit status; every other workload must complete normally.
	local expected=0 nrc grc
	[ "$name" = timeout1 ] && expected=124
	if [ "$ok" = 0 ]; then
		nrc=$(cat "$OUTD/$name/n.rc" 2>/dev/null)
		grc=$(cat "$OUTD/$name/g.rc" 2>/dev/null)
		if [ "$nrc" != "$expected" ] || [ "$grc" != "$expected" ]; then
			ok=1; why="rc native=$nrc guest=$grc expected=$expected"
		fi
	fi
...
trap "gpgconf --kill gpg-agent" 0
echo pw | gpg -q --batch --passphrase-fd 0 --pinentry-mode loopback -c -o b.gpg tree/b/blob &&
echo pw | gpg -q --batch --passphrase-fd 0 --pinentry-mode loopback -d -o b.out b.gpg &&
...
AC
	...
	sums)	run sums "$FIX" dash -c 'sha512sum tree/b/blob && b2sum tree/b/blob && cksum tree/b/blob && md5sum tree/b/blob'
		grade_cmp sums ;;
	lddnames) # Compare file dependencies. The source adapter deliberately
		# omits the kernel-generated vDSO so clocks go through the source.
		# source-clocks independently asserts that absence across exec.
		run lddnames "$FIX" dash -c 'ldd /usr/bin/wc | awk "\$1 != \"linux-vdso.so.1\" {print \$1}" | sort'
		grade_cmp lddnames ;;
...
print(data.decode(), os.waitstatus_to_exitcode(st))'
	...
	gdbbt)	# ptrace INSIDE a guest: gdb runs the fresh binary to a breakpoint
		if [ -x "$OUTD/gcc/gst/wd/hello" ]; then
			...
			if grep -Eq 'Breakpoint [0-9]+,.*\bmain\b' "$OUTD/gdbbt/g.out" &&
			   grep -q BROKE-AT-MAIN "$OUTD/gdbbt/g.out"; then verdict gdbbt 0 ok
...
# A known gap is still a failing test. Keep its label for diagnosis, but never
# let a successful harness exit stand in for a completely passing campaign.
[ "$fail" = 0 ] && [ "$xfail" = 0 ]
