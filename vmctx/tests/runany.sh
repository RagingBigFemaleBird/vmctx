...
# A PASS printed before a deadlock is still a failed execution.
python3 - "$OUT/cleanup.json" <<'PY' || exit 3
import json, sys
try:
    result = json.load(open(sys.argv[1]))
    assert result['complete'], result
except (OSError, ValueError, KeyError, AssertionError) as exc:
    print('runany: cleanup was not verified:', exc, file=sys.stderr)
    raise SystemExit(1)
PY
python3 "$HERE/tests/run-diagnostics.py" "$OUT/home.log" "$OUT/remote.log" \
	> "$OUT/diagnostics.json" || exit 125
[ ! -f "$OUT/.timedout" ] || exit 124
