...
> **Dated 2026-09-06 (session 47).** The bug ledger (§10) ends at #38; the
> September-2026 audit's fixes are recorded per kernel build and per
> frozen userspace tree in `audit/README.md` and the design ledgers under
> `doc-archive/audit-20260906-09/` (start with
> `executor-mm-design-20260909.md` and `HANDOFF-20260909-locks.md`), its
> open review items in `audit/review-obligations.md`. Table names,
> switches and protocol words below are the session-47 ones; where the
> audit renamed a mechanism, the header of the same name under `user/`
> is the current text.
...
- VMHOME_NATIVE_LOADER (source, default 0) — 1 runs the FIRST image's
  dynamic loader natively on the source under ptrace to a breakpoint at
  AT_ENTRY and hands over at the program's `_start` (the project's
  original shortcut: faster start-up, and an architectural break -- the
  first image's relocations, IFUNC choices and library constructors ran
  on the source CPU while every exec'd child's run in the guest; measured
  as OpenSSL's SHA-NI probe on the Ryzen and `sha1msg1` #UD on the Intel
  guest). 0 hands over at the post-exec stop, the loader runs in the
  guest, the same point exec'd children use (owner's ruling, session 47:
  develop and gate with the correct architecture first). Gates at 0:
  loopback suite ×3 120/0, progs 69/0 with gdbbt -- a known gap -- now
  passing.
...
33. **The readahead's claim outlived its landing** (source, `prefetch_settle`;
    netsurf cross-machine parked pre-paint, "the source has been claiming
    this page for 2001 faults in a row"). The window claims each page
    (`PG_CLAIM`) before its GET; when the landing found the page already
    present (`ctx_land` -EEXIST -- usually a NONE-record page this side
    held all along) the settle `continue`d with the record still CLAIM,
    and every guest pull of that page was answered CLAIMING for the rest
    of the run. This was session 42b's "stuck claim" stall; it only shows
    where readahead is on (a LAN). Fix: on EEXIST the record is settled to
    its prior state, or HOME when the prior was THEIRS (a landing by
    another path).

34. **A service task's death by signal never reached the destination**
    (protocol VMR6, `vmr_rsp.ended`/`ended_status`; source `ctx_forward`
    failure path; destination `forward` + `child_thread`). netsurf's parent
    SIGKILLs a helper (kill(2) forwarded); firefox's content process
    abort()s (tgkill forwarded). The service task died, the source answered
    the guest's next calls -EIO ("ctx_call FAILED ... state=Z"), and the
    guest's contexts spun on futex(-EIO) for ever -- netsurf's main thread
    waiting on them, no paint. Fix: when the failed call's task is a
    zombie or gone, the reply carries its wait status (read from
    /proc/<pid>/stat field 52; SIGKILL when already reaped) and the
    destination ends THAT context on the source's word, reporting the
    source's status home and not counting it as its own context death. A
    first form ended every member of the address space on one thread's
    word and took a live netsurf down with its normally exiting threads
    (a zombie thread's exit_code 0 was also mis-read as SIGKILL): both
    corrected -- one context per word, status 0 stays 0.

35. **The construction repair ignored a PROT_NONE mapping** (destination
    `declined_at`; firefox loopback, main context -14 at 0x7ffff771ffe0
    three runs in a row). A cross-task SET is refused by design, so a
    sibling context learns another context's construction only through
    the repair: a fault that would decline, whose task's mapping is not
    what a LIVE row says, replays the row's map. The gate asked TRYFAULT
    and repaired on EFAULT only; TRYFAULT answers EACCES for a mapping
    without the right -- exactly the PROT_NONE jemalloc reservation the
    faulting context still carried after a THREAD had MAP_FIXED'd
    read-write over it. Fix: EACCES repairs too (and the NO-VMA
    instrument names both shapes). Measured: 8 repairs per firefox start,
    the -14 gone.

36. **The first context stood in for a foreign address space** (source
    page service, the CTXPAGE "context is gone" fallback). A gone
    context's ask was answered through a live sibling of its address
    space, and with none left, through ctx0 -- the PARENT's mm for a fork
    child's thread (firefox's forkserver child had mapped a library where
    the parent has a PROT_NONE reservation: "no such address", the child
    dead). Fix: ctx0 stands in only when `as_of` agrees; an address space
    with no live member has ENDED (-ESRCH).

37. **A protection change had no construction row** (destination
    `reply_note_map`'s row noting; netsurf loopback, a helper thread -14
    at the page one past its glib arena's old end, every run). glibc grows
    a non-main arena by mprotect(rw) over its PROT_NONE reservation (and
    jemalloc commits chunks the same way); the change reached only the
    calling context, and a sibling's first touch of the grown page faulted
    against its own PROT_NONE mapping with NO row to repair from ("NO-VMA
    ... a mapping without the right ... unrepairable"). Rows were noted
    for SET replies only. Fix: a PROT reply is noted too (op PROT, the new
    prot), so #35's repair replays the mprotect into the faulting task --
    for the REPAIR ONLY. The first form let PROT rows also keep vacates
    off (the VACATE-LATE skip) and took netsurf loopback from 2/3 to 0/3:
    an mprotect's stamp is the counter at its drain, which can equal an
    unrelated munmap's event seq from another thread, and nothing about an
    mprotect orders it against that unmap (an mmap's address does). PROT
    rows are skipped by the ordering scans.

38. **The first image's loader ran on the source** (source `oracle_start`;
    session 47, the owner's ruling). vmhome ran ld.so natively under
    ptrace to a breakpoint at AT_ENTRY and handed over at `_start`, so the
    first image's relocations, IFUNC selection and every library
    constructor executed on the source CPU while exec'd children's ran
    in the guest -- one process family, two rules. Cross-machine it
    surfaced as OpenSSL probing the Ryzen's SHA-NI and the i7-7700K guest
    executing `sha1msg1` (#UD). The hand-over is now at the post-exec
    stop by default (`VMHOME_NATIVE_LOADER=1` keeps the shortcut); the
    vDSO-hiding auxv patch already lived at that stop. Gated at the new
    default: loopback suite ×3 120/0, progs 69/0 (gdbbt, a known gap,
    passes: gdb's inferior now starts where gdb expects it).
