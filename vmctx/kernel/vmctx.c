 ...
 */
#include "vmctx_cpu_model.h" /* canonical guest identity and feature contract */
...
/* The model is immutable once installed. Check the actual execution CPU at
 * every entry: CPU migration/hotplug must not make a negotiated bit a lie.
 * The caller has disabled preemption. */
static int vmctx_guest_cpu_check(void)
{
	struct vmctx_task *vc = current->vmctx;
	struct vmctx_cpu_model caps;

	if (!vc || !(vc->flags & VMCTX_FLAG_USERCODE)) return 0;
	if (!smp_load_acquire(&vc->cpu_model_ready)) return -EPROTO;
	vmctx_cpu_caps_current(&caps);
	return vmctx_cpu_model_subset(&vc->cpu_model, &caps) ? 0 : -EOPNOTSUPP;
}

static unsigned long vmctx_guest_cr4(unsigned long cr4)
{
	struct vmctx_task *vc = current->vmctx;
	if (!vc || !(vc->flags & VMCTX_FLAG_USERCODE)) return cr4;
	/* These stateful facilities are outside model version 1. */
	cr4 &= ~(X86_CR4_FSGSBASE | X86_CR4_PKE | X86_CR4_CET);
	if (!(vc->cpu_model.leaf1_ecx & VMCTX_CPU_BIT(26)))
		cr4 &= ~X86_CR4_OSXSAVE;
	return cr4;
}

/* XCR0 is not switched by VM entry/exit. Change it only with IRQs and
 * preemption disabled, after restoring the task's FPU and before guest entry.
 * Restore the host value before any host FPU save or interrupt can run. */
static u64 vmctx_guest_xcr0_enter(void)
{
	struct vmctx_task *vc = current->vmctx;
	u64 host;
	if (!cpu_feature_enabled(X86_FEATURE_XSAVE)) return 0;
	host = xgetbv(0);
	if (vc && (vc->flags & VMCTX_FLAG_USERCODE) && host != vc->cpu_model.xcr0)
		xsetbv(0, vc->cpu_model.xcr0);
	return host;
}

static void vmctx_guest_xcr0_leave(u64 host)
{
	if (host && xgetbv(0) != host) xsetbv(0, host);
}
...
static void vmctx_report_gp(struct task_struct *t, u64 rip, u64 rsp, u64 err)
{
	 ...
	 */
	{
		...
		if (boot_cpu_has(X86_FEATURE_XSAVE))
			xcr0 = xgetbv(0);
		pr_warn(VMCTX_PR "  host cr0=0x%llx cr4=0x%llx xcr0=0x%llx (xsave %d, avx %d, osxsave %d)\n",
			rd_cr0(), rd_cr4(), xcr0,
...
/* Shared with vmctx_svm.S; reject layout drift at compile time. */
#include "vmctx_svm_offsets.h"
static_assert(offsetof(struct svm_gctx, vmcb_pa) == VMCTX_SVM_VMCB_PA);
static_assert(offsetof(struct svm_gctx, host_vmcb_pa) == VMCTX_SVM_HOST_VMCB_PA);
static_assert(offsetof(struct svm_gctx, rbx) == VMCTX_SVM_RBX);
static_assert(offsetof(struct svm_gctx, rcx) == VMCTX_SVM_RCX);
static_assert(offsetof(struct svm_gctx, rdx) == VMCTX_SVM_RDX);
static_assert(offsetof(struct svm_gctx, rsi) == VMCTX_SVM_RSI);
static_assert(offsetof(struct svm_gctx, rdi) == VMCTX_SVM_RDI);
static_assert(offsetof(struct svm_gctx, rbp) == VMCTX_SVM_RBP);
static_assert(offsetof(struct svm_gctx, r8) == VMCTX_SVM_R8);
static_assert(offsetof(struct svm_gctx, r9) == VMCTX_SVM_R9);
static_assert(offsetof(struct svm_gctx, r10) == VMCTX_SVM_R10);
static_assert(offsetof(struct svm_gctx, r11) == VMCTX_SVM_R11);
static_assert(offsetof(struct svm_gctx, r12) == VMCTX_SVM_R12);
static_assert(offsetof(struct svm_gctx, r13) == VMCTX_SVM_R13);
static_assert(offsetof(struct svm_gctx, r14) == VMCTX_SVM_R14);
static_assert(offsetof(struct svm_gctx, r15) == VMCTX_SVM_R15);
asmlinkage void vmctx_svm_enter(struct svm_gctx *ctx);
...
static void svm_build_vmcb(struct svm_run *r, const u8 *payload, size_t len)
{
	...
	c->intercepts[INTERCEPT_CPUID / 32]    |= 1u << (INTERCEPT_CPUID % 32);
...
{
	u64 efer_saved, vm_hsave_saved, host_xcr0;
	unsigned long flags;
	int ret = 0;
	...
	ret = vmctx_execution_enter();
	if (ret)
		return ret;
	preempt_disable();
	vmctx_load_task_fpu();
	local_irq_save(flags);
	ret = vmctx_guest_cpu_check();
	if (ret) goto out;
	((struct vmcb *)r->vmcb)->save.cr4 =
		vmctx_guest_cr4(((struct vmcb *)r->vmcb)->save.cr4);
	...
	host_xcr0 = vmctx_guest_xcr0_enter();
	vmctx_svm_enter(&r->gctx);
	/*
	 ...
	 */
	vmctx_guest_xcr0_leave(host_xcr0);
...
/*
 * The guest runs at CPL 3 in the execution task's address space. Every
 * intercepted SYSCALL publishes the architectural frame to its monitor.
 * Call identifiers and termination semantics belong to the source OS.
 */
...
static void svm_build_vmcb_usercode(struct svm_run *r, u64 entry, u64 stack)
{
	...
	/* AMD APM 15.12.2: ICEBP bypasses the #DB exception intercept. */
	c->intercepts[INTERCEPT_ICEBP / 32]    |= 1u << (INTERCEPT_ICEBP % 32);
	/*
...
static int vmctx_back_fault(struct task_struct *t, struct vmctx_task *vc, u64 addr)
{
	...
	/* The native binding retains the object. The execution task need not
	 * own the monitor's descriptor, and descriptor reuse cannot retarget it. */
	bf = vmctx_object_file(vc, false);
...
static int vmctx_be_run_usercode_once(struct task_struct *t)
{
	...
	if (ec == SVM_EXIT_CPUID) {
		u32 a, b, c, d;
		u64 next = v->control.next_rip;
		if (next <= v->save.rip || next - v->save.rip > 15)
			return -EOPNOTSUPP;
		vmctx_cpu_model_cpuid(&vc->cpu_model, v->save.rax, r->gctx.rcx,
			&a, &b, &c, &d);
		v->save.rax = a; r->gctx.rbx = b; r->gctx.rcx = c; r->gctx.rdx = d;
		v->save.rip = next;
		r->last_sysnr = -1;
		return VMCTX_RUN_CONTINUE;
	}
	...
	if (ec == SVM_EXIT_EXCP_UD) {     /* guest SYSCALL -> #UD             */
		...
		/* The source may replace the entire frame (signal return, handler,
		 * exec or debugger). Compare the loaded state with its answer, not
		 * with an assumed sequential return address. */
		if (v->save.rip != regs->ip)
			pr_warn_ratelimited(VMCTX_PR "pid %d: source returned rip 0x%lx but VMCB contains 0x%llx\n",
					task_pid_nr(t), regs->ip, v->save.rip);
	...
	if (ec == SVM_EXIT_EXCP_PF) {     /* guest page fault: demand paging */
		...
		if ((r->unrec_addr != (addr & PAGE_MASK) || r->unrec_n < 2) &&
			...
			if (again == 1) {
				vc->faults++;
				/* SETREGS in this reply has the same authority as
				 * on the first report. The wrapper republishes the
				 * VMCB on return, so preserve the reply there first. */
				ptregs_to_guest_tag(r, regs, WTAG_PF_SERVED);
				return VMCTX_RUN_CONTINUE;
			}
			if (again < 0) {
	...
	if ((ec >= SVM_EXIT_EXCP_BASE && ec <= SVM_EXIT_EXCP_BASE + 31) ||
	    ec == SVM_EXIT_ICEBP) {
		bool icebp = ec == SVM_EXIT_ICEBP;
		u64 vec = icebp ? 1 : ec - SVM_EXIT_EXCP_BASE;
		int rc;

		/* The instruction intercept precedes ICEBP's trap and does not
		 * update DR6. Decode assist accounts for instruction prefixes;
		 * stale DR6 causes must not become part of this exception. */
		if (icebp) {
			u64 next = v->control.next_rip;
			if (next <= v->save.rip || next - v->save.rip > 15)
				return -EOPNOTSUPP;
		 ...
		 * cannot be true here.)
		 */
		/* #DB has no error code. Its architectural cause is saved DR6;
		 * exit_info_1 is not its status. Reset sticky causes before entry. */
		u64 cause = icebp ? 0 : vec == 1 ? (v->save.dr6 ^ DR6_RESERVED) :
			v->control.exit_info_1;
		if (vec == 1) v->save.dr6 = DR6_RESERVED;
		rc = vmctx_redirect_fault(t, regs, vec, 0, cause);
		if (rc == 1) {
		 ...
		 */
		{
			...
			if (v->save.rip && regs->ip != v->save.rip)
				pr_warn_ratelimited(VMCTX_PR "pid %d: the kernel moved guest rip from 0x%llx to 0x%llx (%+lld) while it was out; orig_ax=%ld ax=0x%lx\n",
					task_pid_nr(t), v->save.rip, (unsigned long long)regs->ip,
...
static DEFINE_PER_CPU(unsigned long, vmx_invept_type);
/*
 * Which guest's VMCS is current on this CPU, or NULL. A VMCS stays active on
 * its CPU even after another VMPTRLD makes it non-current. Forgetting that CPU
 * is not a release: loading the active VMCS on a different CPU is invalid.
 * Keep the entire active lifetime inside a preemption-disabled region and
 * VMCLEAR before re-enabling preemption, both at creation and at every entry.
 */
...
static int vmx_cpu_root_enter(void)	/* preemption disabled by caller */
{
	...
	u64 cr4, fixed0 = 0, fixed1 = ~0ULL, ept_caps;
	u64 required = VMX_EPT_PAGE_WALK_4_BIT | VMX_EPTP_WB_BIT |
		VMX_EPT_1GB_PAGE_BIT | VMX_EPT_INVEPT_BIT;

	if (this_cpu_read(vmx_root_on))
		return 0;
	if (!reg)
		return -ENOMEM;
	/* These are local execution prerequisites, not guest CPUID features.
	 * Validate each CPU before using our fixed EPT format on it. */
	if (rdmsrq_safe(MSR_IA32_VMX_EPT_VPID_CAP, &ept_caps) ||
	    (ept_caps & required) != required ||
	    !(ept_caps & (VMX_EPT_EXTENT_CONTEXT_BIT | VMX_EPT_EXTENT_GLOBAL_BIT)))
		return -EOPNOTSUPP;
	this_cpu_write(vmx_invept_type,
		(ept_caps & VMX_EPT_EXTENT_CONTEXT_BIT) ?
		VMX_EPT_EXTENT_CONTEXT : VMX_EPT_EXTENT_GLOBAL);
...
/* VM entry/exit without VPID discards linear translations, but does not
 * discard EPT paging-structure caches. An allocator can reuse an EPT root
 * physical address while those caches still point to its freed lower table.
 * Flush on this CPU before every entry, including after migration or root
 * reuse. This needs no inference about where a previous owner ran. */
static int vmx_invalidate_ept(u64 eptp)
{
	unsigned long type = this_cpu_read(vmx_invept_type);
	struct { u64 eptp, reserved; } operand = {
		.eptp = type == VMX_EPT_EXTENT_CONTEXT ? eptp : 0,
	};
	u8 err;

	asm volatile("invept %[desc], %[type]; setna %[err]"
		     : [err] "=qm"(err)
		     : [desc] "m"(operand), [type] "r"(type)
		     : "cc", "memory");
	if (err) {
		pr_err_ratelimited(VMCTX_PR "vmx: INVEPT type %lu failed for EPTP 0x%llx\n",
				   type, eptp);
		return -EIO;
	}
	return 0;
...
/* Shared with vmctx_vmx.S; reject layout drift at compile time. */
#include "vmctx_vmx_offsets.h"
static_assert(offsetof(struct vmx_gctx, rax) == VMCTX_VMX_RAX);
static_assert(offsetof(struct vmx_gctx, rbx) == VMCTX_VMX_RBX);
static_assert(offsetof(struct vmx_gctx, rcx) == VMCTX_VMX_RCX);
static_assert(offsetof(struct vmx_gctx, rdx) == VMCTX_VMX_RDX);
static_assert(offsetof(struct vmx_gctx, rsi) == VMCTX_VMX_RSI);
static_assert(offsetof(struct vmx_gctx, rdi) == VMCTX_VMX_RDI);
static_assert(offsetof(struct vmx_gctx, rbp) == VMCTX_VMX_RBP);
static_assert(offsetof(struct vmx_gctx, r8) == VMCTX_VMX_R8);
static_assert(offsetof(struct vmx_gctx, r9) == VMCTX_VMX_R9);
static_assert(offsetof(struct vmx_gctx, r10) == VMCTX_VMX_R10);
static_assert(offsetof(struct vmx_gctx, r11) == VMCTX_VMX_R11);
static_assert(offsetof(struct vmx_gctx, r12) == VMCTX_VMX_R12);
static_assert(offsetof(struct vmx_gctx, r13) == VMCTX_VMX_R13);
static_assert(offsetof(struct vmx_gctx, r14) == VMCTX_VMX_R14);
static_assert(offsetof(struct vmx_gctx, r15) == VMCTX_VMX_R15);
static_assert(offsetof(struct vmx_gctx, fail) == VMCTX_VMX_FAIL);
static_assert(offsetof(struct vmx_gctx, launched) == VMCTX_VMX_LAUNCHED);
asmlinkage void vmctx_vmx_enter(struct vmx_gctx *ctx);
...
static int vmx_make_current(struct vmx_run *r)
{
	...
	if (r->loaded_cpu >= 0 && r->loaded_cpu != cpu)
		smp_call_function_single(r->loaded_cpu, vmx_clear_here, r, 1);
	/* A non-current VMCS would still be active on this CPU. */
	if (cur && cur != r)
		vmx_clear_here(cur);
	this_cpu_write(vmx_cur, NULL);
...
static int vmx_enter_once(struct vmx_run *r)
{
	...
	if (ret)
		goto out;
	ret = vmx_invalidate_ept(virt_to_phys(r->ept[0]) |
				 VMX_EPTP_WB | VMX_EPTP_WALK4);
	if (ret)
		goto out;
	...
	vmcs_write(GUEST_CR4, vmctx_guest_cr4(rd_cr4() | X86_CR4_VMXE));
	...
	case EXIT_REASON_EXCEPTION_NMI:
		...
		/* #PF carries CR2; #DB carries the architectural debug cause. */
		if ((r->intr_info & 0xff) == 14 ||
		    (r->intr_info & 0xff) == 1)
		 ...
		 * software exceptions (type 6: INT3, INTO), privileged software
		 * exceptions (type 5: ICEBP), and software interrupts (type 4).
		 * It was read only
	...
	r->gctx.launched = 1;
out:
	/* Neither scheduling nor task migration may carry an active VMCS out
	 * of this CPU-local critical section. Capture guest state above first. */
	vmx_release(r);
	preempt_enable();
	vmctx_execution_leave();
	return ret;
...
static int vmx_run_usercode_once(struct task_struct *t)
{
	...
	case EXIT_REASON_CPUID: {
		...
		vmctx_cpu_model_cpuid(&vc->cpu_model, (u32)r->gctx.rax,
			(u32)r->gctx.rcx, &a, &b, &c, &d);
		r->gctx.rax = a;
		r->gctx.rbx = b;
		r->gctx.rcx = c;
		r->gctx.rdx = d;
	...
	case EXIT_REASON_XSETBV:
		/* XSETBV is privileged; the user guest receives source-native #GP. */
		vector = 13;
		r->exit_qual = 0;
		goto guest_exception;
	case EXIT_REASON_EXCEPTION_NMI:
	...
	guest_exception:
		...
		if (((r->intr_info >> 8) & 7) != 2 && vector < 32) {
			 ...
			 * SVM arm's note. VMX leaves RIP on both INT3/INTO
			 * (type 6) and ICEBP (type 5).
			 */
			...
			vmx_guest_to_ptregs(r, regs);
			/* VMX reports #DB status in exit qualification, already in
			 * positive polarity. No source signal meaning is assigned. */
			rc = vmctx_redirect_fault(t, regs, vector, 0,
						  vector == 1 ? r->exit_qual : r->intr_error);
			if (rc == 1) {
				vmx_ptregs_to_guest(r, regs);
				return VMCTX_RUN_CONTINUE;
			}
...
static int vmx_be_create(struct task_struct *t)
{
	...
	/*
	 * Creation, like every entry, releases the active VMCS before the task
	 * can move to another CPU. Each entry reloads that CPU's host state.
	 */
	vmx_release(r);
	preempt_enable();
	if (ret) {
...
static int __init vmctx_init(void)
{
	...
	if (g_caps.vendor == VMCTX_VENDOR_AMD &&
	    (g_caps.caps & VMCTX_CAP_ENABLED)) {
		ret = vmctx_register_execution_backend(&vmctx_svm_backend);
		if (ret)
		...
		if (ret) {
			...
			ret = vmctx_register_execution_backend(&vmctx_vmx_backend);
			if (ret) {
