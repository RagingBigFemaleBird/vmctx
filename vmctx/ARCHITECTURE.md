...
> **Dated 2026-09-06 (session 47).** The September-2026 audit (HANDOFF.md
> §2) kept every principle here and restructured the code beneath them:
> userspace is `vmhome.c`/`vmremote.c` plus thirty-eight headers that
> name the contracts (source admission, custody, lineage, registry,
> transfer; execution targets, MM lifetimes, shared pages, page receipts
> and settlement, monitor diagnostics), the module is `vmctx.c` +
> `vmctx_backend.c` + the SVM/VMX entry `.S` files, the wire protocol is
> VMRP v25 (exact retained transfer episodes, full CPU state, a
> negotiated CPU model, opaque MM identities), and the kernels gained the
> controls listed in `kernel-patches/README.md`. The ownership and
> execution contracts as they now stand are `audit/ownership.md`; what is
> measured and what is open is `audit/README.md`. Read this file for the
> shape of the system and those for its current letter.
