...
- **Gates** (session 47, srcid 44d1b78e4e3f, the loader IN THE GUEST --
  `VMHOME_NATIVE_LOADER` off by default since the owner's ruling): suite
  loopback x3 **120/0**, tests/progs.sh **69/0** with gdbbt (a known gap)
  now passing, netsurf loopback 3/3, cross **40/0**; the browser demo GREEN (links2 calibrated, session
