...
# The kernel tree the module is built against: the RUNNING kernel's own build
# tree (since the September-2026 audit that is ~/audit-20260907/<build>-kernel-7.0.14,
# reached through /lib/modules/$(uname -r)/build), never the old
# ~/vmctx-build/linux-7.0.14 checkout, which is stale and not a git tree.
KTREE=${KTREE:-/lib/modules/\$(uname -r)/build}
# The module the box runs, for verify-build. A rebuilt module (MODULE=1) is
# staged beside it as ~/vmctx-<release>.ko and is NOT loaded by this script.
AMD_KO=${AMD_KO:-/home/biwu/audit-20260907/terminal222-native/vmctx.ko}
export AMD_KO
...
# The core header used to be pushed into the box's kernel tree here. That was
# right when the box built its module against a plain checkout; it is wrong
# now that the box's build tree IS the verified source of its running kernel
# (a header pushed into it silently changes what the next module build sees,
# and the tree carries a dozen vmctx headers, not one). Kernel sources travel
# the other way -- kernel-patches/ is regenerated from the verified tree -- so
# this sync is off unless asked for explicitly.
if [ "${SYNC_KHEADER:-0}" = 1 ]; then
...
if [ "${MODULE:-0}" = 1 ]; then
	...
	# Staged, not loaded: the loaded module is part of the deployment of
	# record (its srcversion is what verify-build compares), and swapping it
	# under a box that may be running guests is an operator's decision.
	$SSH "SUDO_PASS=${AMD_SUDO_PASS:-biwu} SUDO_ASKPASS=\$HOME/askpass.sh sudo -A bash -c '
	  cd /home/biwu/lan-boot/vmctx/kernel &&
	  rm -f vmctx.ko vmctx.o vmctx.mod.* *.o && touch vmctx.c vmctx_backend.c &&
	  make KDIR=$KTREE 2>&1 | tail -3 &&
	  modinfo ./vmctx.ko | awk \"/^vermagic|^srcversion/\" &&
	  cp -f ./vmctx.ko /home/biwu/vmctx-\$(uname -r).ko &&
	  echo \"staged as /home/biwu/vmctx-\$(uname -r).ko (loaded: \$(cat /sys/module/vmctx/srcversion 2>/dev/null))\"'"
fi
