...
One file. §0 is the state to resume from; §1 and §2 are the two most recent
sessions (the 2026-09-10 consolidation and the 2026-09-06 → 09-09
architecture audit); §3–§7 are the ledger that the 2026-09-04 consolidation
built (sessions 27–47), kept verbatim because the numbers and the refuted
designs in it are still the ones that matter. Originals: the per-session
handoffs under `doc-archive/handoffs/` (sessions 27–46 individually,
27–47 as the consolidated file this one grew from), and the audit's
twenty-seven working documents under `doc-archive/audit-20260906-09/`
(hash-verified against the archive manifest beside them). The other
documents: RUNNING.md (operator's guide), ARCHITECTURE.md (the map, as of
session 47), AUDIT.md (every flow, the corner catalog, the bug ledger §10
to #38, the switch reference §9 — as of session 47), `audit/README.md`
(the validation record of the audit: deployed hashes, exact results,
evidence, remaining work), `audit/ownership.md` (the ownership and
execution contracts, and the required native-residency design),
`audit/review-obligations.md` (the consolidated open review items),
NOTES.md (campaign history with the numbers behind every refuted design),
HISTORY.md (the arc), PITFALLS.md (the traps), REDESIGN.md (the ownership
design record through session 36).

## 0. START HERE — state as of 2026-09-10 (session 49)

**Not all green, committed anyway on the owner's instruction.** The owner
directed on 2026-09-10 that the audit's tree be committed and pushed as it
stands, with the intermediate documents summarized, the kernels squashed to
base + one commit, the repository squashed to one commit, and the build and
tests rerun — with NO code change. That is what this session did. The
failures the audit left open are still open and are listed below; nothing
here claims they passed.

**What is deployed, and that the tree IS what is deployed.**

| where | kernel | module | userspace |
|---|---|---|---|
| 10.0.0.229 (AMD, source + loopback lab) | **#222** `7.0.14-vmctx-audit-recall1-terminal222`, boot `fca91ceb-77d2-4bed-842d-b4f3037301d5`, built from `~/audit-20260907/mapping-kernel-7.0.14` | `~/audit-20260907/terminal222-native/vmctx.ko`, srcversion `665E2B59F7ADD5DFE313DB5` | `~/lan-boot/vmctx/build/{vmhome,vmremote-local}` id **75915bd28abd** (rebuilt from this tree today; the audit's frozen `~/audit-20260907/mm-settle44-user-tree` has the same id) |
| 10.0.0.30 (Intel, netboot destination) | **#71** `6.18.35-0-lts-audit-readobj71`, boot `e8e036f7-5adc-4000-b0c2-abd4ba2c2306`, reached by kexec from the #63 recovery boot | `/root/audit-readobj71/vmctx.ko`, the same srcversion | `/root/audit-readobj71/vmremote-settle44` id 75915bd28abd (470/471); process owner `/root/audit-readobj71/controls/native-run-descendants43` |

Proven today, not assumed: every file of the AMD build tree that differs
from vanilla 7.0.14 is hash-identical to `src/linux-7.0.14` HEAD (the local
tree had been older — "AMD's nested git kernel tree is stale" in the audit's
words — and was brought up from the box's files, 38 modified + 29 added +
`.config`); `src/linux-6.18.35` HEAD is byte-identical to the Intel #71
frozen inputs and its `arch/x86/boot/bzImage` is the deployed image; the
module built here from `vmctx/kernel/` has the srcversion both boxes have
loaded; `make -C vmctx/user` reproduces id 75915bd28abd. `./verify-build.sh
amd` says "everything deployed matches this tree" with the new srcversion
checks; `./verify-build.sh intel` checks the audit paths (it defaults to
them now).

**The Intel box's boot is NOT the repository's boot.ipxe.** The lab servers
running here are a `dnsmasq` on the recovery config and `python3 -m
http.server 8080` rooted at `/mnt/d/VM/lan-boot-recovery-20260908/http`
(the storage incident of 2026-09-08 moved the lab there — §2). That root's
`boot.ipxe` boots the SAFE recovery image `audit-20260907-runtime63-repacked`
(#63, no module auto-loaded); #71 was entered from it by kexec
(`readobj71-intel-kexec-request.json` on the recovery drive has the exact
cmdline). A power cycle of .30 therefore comes up as #63; getting back to #71
means re-running the kexec step with the `audit-20260907-readobj71-repacked`
package that root serves. `http/boot.ipxe` in this repository still names
`vmlinuz-custom` (the pre-audit lab) and is what a checkout's `start.sh`
would serve — do not start it beside the recovery servers.

**Repository.** ONE root commit (this squash), author and committer
`Bi Wu <biwu85@gmail.com>`. Kernels: `src/linux-7.0.14` = `add83c849` +
`adb1174f8` (no remote — `kernel-patches/linux-7.0.14/0001-*.patch` is the
only off-box copy); `src/linux-6.18.35` = `ed9484e8f` + `d2a1974a4`, pushed
to `vmctx-kernel.git` `master` and `port-7014-coherence`. Both patches were
regenerated and PROVEN: applied to the vanilla index they produce the
commit's exact tree (`kernel-patches/README.md` has the recipe).

**Gates run this session** (logs under `doc-archive/gates-20260910/`; all on the deployed identities above; the
harness is the audit's — `owned-run.py` subreaper cleanup, `run-diagnostics.py`
grading every case's home/remote logs, counters read before and after):

- home (WSL): `make -C vmctx/user` all tools at 75915bd28abd; module build
  against `src/linux-6.18.35` (srcversion matches the loaded modules);
  `make -C vmctx/user check-host` **PASS** (85 s).
- AMD: `./sync-amd.sh` (rsync, rebuild, verify all-ok); `check-host`
  **PASS** (114 controls) once the shell's open-file limit was raised — the
  descriptor control dup2()s to fd 8192 and the box's login limit is 1024
  (suite.sh now raises it; the first run said only "FAIL execution
  descriptors case 0").
- AMD loopback suite ×1 (78 cases, serial, kernel #222, module of record):
  **78 passed, 0 failed**, 5 min 14 s, counters [0,0,0,1,1,0,3] before and after, boot unchanged, no kernel warnings (2026-09-10 00:21–00:26).
- AMD → Intel cross suite ×1 (`RUNANY=tests/runany-x.sh`, destination
  executor and owner at the `/root/audit-readobj71` paths):
  **78 passed, 0 failed**, 7 min 28 s (00:29–00:36), source counters [0,0,0,1,1,0,3] and destination counters [0,0,0,1,1] before and after, both boot ids unchanged, no kernel warnings on either side; every case's destination cleanup report complete with no survivors.
- `tests/progs.sh` (the 49-shape breadth campaign): **68 pass, 1 fail, 0 skip; known gaps: gdbbt still failing (no breakpoint hit), gpgsym and stracec passing today** (7 min, 00:37–00:44, counters unchanged, boot unchanged). The one failure is **nodejit**: node exits 1 with empty output; remote.log says one context ended on its own with status 131 and its joiner was woken as though it had finished, home.log shows the service task killed by signal 9 -- the Node thread-teardown class the audit still listed (nodework passed this time). progs.sh returns non-zero for a known gap still failing, by design.

**Still failing / open (from `audit/README.md`, unchanged by this session):**

1. Native remote residency: a remotely owned page is an ordinary absent PTE
   beside a separate record, so UFFDIO_COPY/ZEROPAGE install a second page,
   MADV_COLLAPSE zero-fills, MREMAP_FIXED loses provenance (all four native
   probes FAIL on #222). The required design is `audit/ownership.md`.
2. Foreign GUP (ptrace software breakpoint: INT3 restored on the source, the
   executor still traps) — a custody hole, not a flush.
3. Secondary-TLB revocation is not atomic-safe; shared-object aliases in
   unadopted MMs and async writers are outside the owner's scope.
4. GDB under vmctx: SIGFPE before main (gdbbt), the standing ptrace-semantics
   gap; gpgsym/stracec are the other two KNOWN_GAPS of progs.sh (both
   passed today). nodejit failed today with a context dying 131 during
   teardown (the Node thread class; nodework passed) -- a residual, not a
   known gap, and the only red line of the breadth run.
5. Firefox interaction and normal-shutdown gates: pending on current inputs
   (`tests/browser-audit.py`, the browser40 fixture).
6. `tests/ownership.c` does not compile against the current `pgstate.h`
   (`pg_try_claim` takes four arguments); suite.sh skips it. Either bring it
   back or delete it — it is the only source in the tree that does not build.
7. The Intel #71 module was built with the previous `vmctx_executor_map.h`;
   rebuild and redeploy the module from this tree on the next Intel cycle.

**What to do next, in order:** the native residency representation
(`audit/ownership.md` §"Required representation", with the four failing
native probes as the oracles), then foreign GUP fault service, then the
atomic-safe secondary-TLB protocol on both SVM and VMX — each gated by the
native control campaign (57 AMD / 26 Intel today), the 78-case suite loopback
and cross, then breadth and the browser. Hashes and generations remain
diagnostics, never authority (the owner's rule, §2).

## 1. Session 49 (2026-09-10) — consolidation, squash, gates; no code change

Directive (verbatim in spirit): iterate through all files, commit and push;
summarize all intermediate work documents; squash each kernel into base +
one commit and the repository into one commit; rebuild and run the tests;
documentation and scripts only, no code.

What was found on arrival: the audit's tree uncommitted (24 modified files,
~130 new: 38 userspace headers, the module split into `vmctx.c` +
`vmctx_backend.c` + SVM/VMX entry `.S`, 124 native/host controls under
`tests/control/`, twelve Python harness tools, the `audit/` triad); 46 + 29
working-tree kernel patches; `src/linux-7.0.14` older than the box's build
tree; `src/linux-6.18.35` equal to Intel #71; HANDOFF.md cut to 32 lines
with the twenty-seven audit documents archived to the recovery drive only.

Done:

- **Kernel truth established by hashing, not by trust.** Every file of the
  AMD box's `mapping-kernel-7.0.14` was git-hashed (107,639 files) and set
  against the vanilla and committed trees: 38 vanilla files changed, 29
  vmctx files added, `.config` new; the local tree lacked 8 of them and had
  older versions of 9. The box's files were copied in, every hash rechecked,
  the stray root-level `memory.c`/`vmctx.c` and unreferenced
  `kernel/vmctx.h` left out. The Intel side needed nothing (21/21 identical,
  bzImage sha matches the deployed vmlinuz).
- **Squashed.** 7.0.14: `add83c849` + `adb1174f8` (the old commit's message
  extended with the audit's source-side contracts; a patch file that had
  been committed INSIDE the kernel tree in 2026-08 dropped). 6.18.35:
  `ed9484e8f` + `d2a1974a4`, pushed with `--force-with-lease` to both remote
  branches. One patch per tree regenerated and proven by
  `read-tree` + `apply --cached` + `write-tree` equality. The 46 + 29
  intermediate patches and the `support/` headers removed; their per-step
  descriptions kept verbatim in
  `doc-archive/audit-20260906-09/kernel-patches-README-presquash-20260910.md`.
- **Documents.** The 27 archived audit documents restored into the
  repository under `doc-archive/audit-20260906-09/` (sha256-verified against
  `scratch-documents-before-consolidation42.json`); the consolidated
  sessions-27–47 HANDOFF kept as `doc-archive/handoffs/HANDOFF-sessions27-47-consolidated.md`
  and folded into this file as §3–§7; the five session-47 commit messages in
  `doc-archive/repo-log-session47-presquash.txt`; `kernel-patches/README.md`
  rewritten for the one-patch form; RUNNING.md §3/§7/§10 for the audit
  harness; ARCHITECTURE/AUDIT/HISTORY/PITFALLS given their dated notes.
- **Scripts.** `verify-build.sh` checks the deployment of record (audit
  kernel names, module and executor paths overridable by `AMD_*`/`INTEL_*`,
  and the module's srcversion: loaded vs file vs this tree's build);
  `sync-amd.sh` builds the module against the running kernel's own tree,
  stages it instead of overwriting the deployed one, and no longer pushes a
  header into the box's kernel tree unless `SYNC_KHEADER=1`; `suite.sh`
  raises the open-file limit for the host controls and skips
  `tests/ownership.c`.
- **Gates** as in §0.

Not done, deliberately: no source edit anywhere (the stale `ownership.c`
included); no Intel redeploy; no change to the recovery lab's boot script.

## 2. Session 48 (2026-09-06 → 2026-09-09) — the architecture audit

Run by a different assistant (ChatGPT) from commit `99ecf75`, userspace
`44d1b78e4e3f`, AMD #181, Intel `6.18.35-0-lts` #56, with the directive:
correct, sustained cross-machine Firefox with real interaction, a complete
line-by-line audit, kernel changes authorized, and the owner's later
steering verbatim — *"checksums may serve as a way to determine consistency,
but it should not be relied on. One page can only have one owner at one
time. You need to fix the underlying locks issue first."* Delivery rule:
commit and push only when everything is green on both machines. Four days,
forty-one AMD kernel builds (#182 → #222), ten Intel builds (#62 → #71),
some forty frozen userspace trees, ~2,300 evidence files. It ended NOT
green; `audit/README.md` is its validation record, `audit/ownership.md`
and `audit/review-obligations.md` its conclusions. The twenty-seven working
documents are under `doc-archive/audit-20260906-09/` — read
`HANDOFF-20260909-locks.md`, `continuation-20260909-green.md` and
`executor-mm-design-20260909.md` first; `2026-09-07.md` (100 KB) is the
day-by-day.

**The arc, compressed.**

- *Reading and first repairs (09-06/07).* A read ledger over 148 authored
  files / 69,293 lines (`read-ledger.json`, `reread-20260907.json`).
  Firefox harness with real X input and page acknowledgements
  (`tests/browser-audit.py`). First findings: the SVM register reply, local
  transfer-table capacity, loan and installation records, a failed fork
  leaking the context (kernel patch 0002), fork/signal extended state.
- *Architectural CPU state and a negotiated CPU.* GETCPU/SETCPU carry the
  whole XSAVE state in each adapter's own layout (never another OS's save
  area); the guest sees a negotiated generic x86 model — CPUID answered from
  it on both SVM and VMX, XCR0 following it, a generic vendor identity so
  loaders that need one still find the baseline ISA, ELF hwcaps restricted,
  /proc/cpuinfo reporting it. This is what closes the session-47 SHA-NI
  finding for good. Exceptions and debug traps are delivered in the source
  kernel; clocks stay with the source; execution CPU time is measured by the
  executor and credited to the source task's clocks (RUNTIME/BOUNDARY, the
  gpgsym cputime gap's designed fix), with killable checkpoints.
- *Executor operations named, not implied.* PROTECT_MM and EXECUTOR_MAP_CAPS
  (the executor proves its local operations before START), SET_SHARED_PAGE,
  MAPPING (the VMA and a monitor-owned file reference — no injected fstat),
  MAPOBJ_READ, quiesce leases + PROTECT_BACKING for snapshots, one
  refcounted MMU notifier per mm, `vmctx_map_trace`.
- *Lifetimes retained, identities exact (#204 → #214).* Native syscall
  admission as BEGIN/QUERY/COMMIT tickets (entry tracing and seccomp on the
  canonical frame, committed children published before the parent returns);
  RESTORE bound to the monitor's task lifetime; retained context descriptors
  that survive PID reuse and carry terminal status; ABI-2 MM lifetime
  queries and exact-task SIGNAL; execution objects retained and replaced
  under an epoch fence; the expected-MM MEMORY envelope refusing wrong-MM
  calls before argument access. Userspace followed: execution targets,
  memory identity, source registry/lineage/admission headers; the
  `exec_wipe` removed.
- *Custody (#215 → #217).* Thread-owned transfer tickets on the source;
  reserve before capture; replay bytes kept until the exact idempotent
  ACK; failure retained until native cancellation or abandonment; custody
  abandoned before exit waits; time never confirms delivery. Landing
  requires full writes (no authority from retry counts); connection custody
  ends the session on unconfirmed grants; read-only private pages transfer
  through forced COW then TAKE. The protocol went VMR6 → VMRA v10 → VMRE
  v14 → VMRF v15 → … → VMRP v25 ("exact retained source transfer
  episodes").
- *The storage incident (09-08).* WSL's ext4 went emergency-read-only while
  packaging Intel #63; C: hit zero bytes. Read-only recovery copied the
  project to `/mnt/d/VM/lan-boot-recovery-20260908` with per-file hashes;
  the lab (dnsmasq + HTTP) has run from there since; large artifacts live on
  D:. Intel was NOT rebooted on a changed modloop checksum.
- *Diagnostics that cannot deadlock (diagnostic28).* A guest stderr write
  faulting while the monitor logged to the SAME open file description was a
  measured lock cycle (+10 fault deadlines); `monitor-diagnostics.h` gives
  the fault-service threads a bounded nonblocking channel drained by its own
  thread.
- *The native MM coverage audit (09-09, `mm-path-coverage-20260909.md`).*
  Four read-only transfer classes, UFFD COPY/ZERO, MADV_COLLAPSE and
  MREMAP_FIXED measured against #216 with source controls
  (`readonly-custody.c`, `uffd-custody.c`, `collapse-custody.c`,
  `remap-custody.c`): #217 fixed the read-only classes; the other three
  cannot be fixed while remote residency is `pte_none` + a separate
  xarray — the finding that produced `audit/ownership.md`. HMM was probed
  as a substrate (#219/#220, `test_hmm` release bug fixed, all 11 probes
  pass); it is not integrated.
- *Closing gates (#220 → #222, Intel #70/#71).* retire38 (mapping
  retirement ordered under the layout order, generation checks report and
  never substitute bytes) passed six 1,024-round stress runs but the suite
  77/78: an1's LAND failed EBUSY against a refused capture's guard → #221
  refuses capture over an incumbent CLAIM/TRANSIT; the corrected diagnostic
  oracle regraded two earlier "passing" suites to 72/78. watchwrite40 (a
  sibling's nonpresent write mapped read-only first, then the normal COW
  path) → 78/78 cross on #222 → #71. #222 reads terminal death and monitor
  attachment under one `ev_lock` section (retained QUERY lost authorization
  at native exit 872 of 2,000 on #221). settle44 resolves shared settlement
  and page-service reads through the shared object/offset slot (sm1, sm2,
  the 256-submission io_uring control, both Node workloads pass).
- *Harness.* `owned-run.py` (a subreaper that reaps only its descendants,
  by pidfd and birth time), the native-run owner with `--control-stdin
  --report` on both hosts, `remote-executor.py`/`cross-run.py` for
  destination-owned cleanup reports, `run-diagnostics.py` (a PASS with a
  stale-serve, failed LAND or lost receipt is a failure), `suite-oracle.py`
  and `oracle-failures.py` exercising the graders with failing artifacts;
  the suite grew 40 → 78 cases; progs.sh grades rc strictly and never lets
  a known gap make the campaign green; Makefile targets `check-host`,
  `check-tables`, `check-shared-settlement` run the native controls with no
  kernel.

**Its final state (the validation record, unchanged by session 49):**
AMD #222 57 native controls PASS; Intel #71 26 PASS; loopback and cross
suites 78/78 on settle44; retained-QUERY race 2,000 exits / 429,051 queries
PASS; HMM 11/11; breadth 68/72 on #221 (gpgsym, cpiort, gdbbt, nodework),
then cpiort/gpgsym/nodework fixed by harness or settle44 and **GDB still
failing (SIGFPE before main)**; ptrace INT3 restore FAIL; the four native
residency probes FAIL; Firefox gates pending; final full suites on the
final inputs never re-run. Cumulative counters
[mm_live, refcnt, handles, take_lost, deadline_hits, transfer_live,
transfer_abandoned] = [0,0,0,1,1,0,3] after the native campaign, the
nonzeros being deliberate negative controls; every ordinary run must leave
the first three and the sixth at zero and not increase the others.


## 3. Session 47 (2026-09-06) — the browser campaign: links2 cross-machine on the user's screen; netsurf and firefox pursued

Directive: a bigger browser the user can interact with, executed on the
Intel box with the window on the AMD box's own display; then netsurf and
firefox, loopback first. Delivered: **links2 cross-machine on the AMD
box's logged-in display :0** (`demo/xbrowser.sh` → `xprep.sh` on the
box, which kills only its own port's vmhome — a global `pkill -x vmhome`
in browser-run.sh killed the user's demo once). netsurf rendered
cross-machine once (screenshot on record) and 2/3 on loopback; firefox
starts, repairs and runs ~20–30 s on loopback before the coherence
residual takes it. Six defects found from the corpses, five fixed,
deployed and GATED on srcid 7d1715516402 (loopback suite ×3 **120/0**,
progs **68/1** -- nodework's known ~1/6 thread-list class -- plus the 3
known gaps, cross suite **40/0**, all with the links2 demo running beside
them; `runany-x.sh` now honours VMCTX_SHARED=1 so the cross suite no longer
kills other runs' halves) (AUDIT §10 #33–#37):

- #33 readahead claim leak on EEXIST (the 42b stuck-claim stall, LAN only).
- #34 a service task's death by signal reaches the destination (VMR6:
  `ended`/`ended_status`); one context per word; a normal exit stays 0.
- #35 the construction repair now fires on EACCES (a PROT_NONE
  reservation another thread MAP_FIXED'd over).
- #36 the first context stands in only for its own address space.
- #37 mprotect replies get construction rows (repair only, never
  ordering — the ordering form took netsurf loopback 2/3 → 0/3).
- Found, not fixed: the loader and every library constructor run on the
  SOURCE, so OpenSSL probed the Ryzen's SHA-NI and the i7-7700K guest
  executed sha1msg1 (#UD). Demo workaround `OPENSSL_ia32cap=:~0x20000000`;
  the fix is handing the first image over at the exec stop (ld.so in the
  guest, as exec'd children already do).
- Open: a page stays CLAIMED on the source for the rest of a run
  (0x7ffff04a0000, netsurf's glib arena) on loopback under load with
  readahead on; a newborn thread context dies at birth on VMX with no
  fault line; firefox's remaining deaths are the zero-page/coherence
  residual after ~20 s.

**Session 47, later (owner's ruling): the native loader is a flag, OFF.**
`VMHOME_NATIVE_LOADER` (AUDIT §9, #38). The first image is handed over at
the post-exec stop; ld.so, relocations, IFUNCs and constructors run in the
guest like every exec'd child's. Gated at the new default: loopback suite
×3 **120/0**, progs **69/0** (gdbbt, a known gap, now passes), netsurf
loopback 3/3, cross suite **40/0**, and netsurf renders cross-machine
WITHOUT the OpenSSL SHA mask (its constructors now probe the Intel CPU). The Intel box stopped answering at
15:23 during this round (cause unknowable: a RAM boot keeps no dmesg); the
lab's netboot servers were not running, so it could not come back on its
own. Recovery that worked: `sudo ./start.sh` here (dnsmasq + HTTP), then a
wake-on-LAN magic packet to its MAC 1c:1b:0d:e8:f1:e3 (from the AMD box or
here); it netbooted in ~55 s and the boot fetched the staged build, so
verify-build was all-ok with nothing to deploy.

## 4. State as of 2026-09-04 (session 46b) — superseded by §0, kept for its numbers
...
## 6. The operational rules (each paid for; RUNNING.md and PITFALLS.md carry the full set)
...
## 7. The open frontiers before the audit (session 46b's list and mechanisms; §0 supersedes)
