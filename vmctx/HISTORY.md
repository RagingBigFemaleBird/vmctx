...
As of session 47 (2026-09-06) every gate was green: the 40-case suite
loopback and cross-machine, the pools, the 69-shape breadth harness with
its three named gaps, links2 and netsurf cross-machine, firefox to its
coherence residual.

Then the eleventh arc, 2026-09-06 → 09-09 — the architecture audit
(HANDOFF.md §2): a line-by-line read with kernel changes authorized and
the owner's rule "one page has one owner; fix the locks first, never lean
on checksums". It replaced inferred behaviour with named contracts —
architectural CPU state and a negotiated CPU model, native syscall
admission, retained identities across PID reuse, thread-owned transfer
custody with exact acknowledgements — across forty-one AMD kernel builds
and ten Intel builds, grew the suite to 78 cases graded by a diagnostic
oracle, and ended with the suites 78/78 both ways on #222/#71 but NOT
green: the native remote-residency representation (a remotely owned page
is still an absent PTE beside a record) fails four native probes, GDB
fails before main, the Firefox gates never ran. Session 49 (2026-09-10)
committed that state on the owner's instruction, squashed, and reran the
gates it could (HANDOFF.md §0–§1). The kernel work ships as one patch per
tree in `kernel-patches/`; the open frontiers are `audit/README.md` and
`audit/ownership.md`.
