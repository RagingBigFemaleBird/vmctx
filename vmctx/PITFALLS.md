...
## The September-2026 audit's additions

- **A monitor must never log to a file the guest can hold.** vmhome's
  fault-service thread logged to the same open file description the
  guest's stderr write(2) was faulting through (shared fd 2, one
  `fdget_pos` lock): a measured deadlock, +10 fault deadlines.
  Diagnostics go through their own bounded channel and thread
  (`user/monitor-diagnostics.h`); never reintroduce a synchronous
  stderr write on a fault path.
- **A login shell's open-file limit fails a control silently.** The
  descriptor control dup2()s to fd 8192; biwu's soft limit on the AMD box
  is 1024 and the only symptom was "FAIL execution descriptors case 0".
  suite.sh raises it; a Makefile target run by hand needs `ulimit -n
  16384` first.
- **A header pushed into the box's kernel tree changes the next module
  build without a word.** The box's build tree is now the verified source
  of its running kernel; sync-amd.sh no longer touches it. Kernel sources
  travel the other way (box → `src/` → patch), by hash comparison.
- **The local kernel tree can be OLDER than the deployed one and look
  fine.** The audit built #222 from its own tree on the box while
  `src/linux-7.0.14` sat at an earlier step; nothing in `git status`
  says so. Before a kernel squash, git-hash every file of the box's tree
  against vanilla and against HEAD (HANDOFF.md §1).
- **The storage incident (2026-09-08).** WSL's ext4 went read-only with C:
  at zero bytes during a packaging step. The lab has run from
  `/mnt/d/VM/lan-boot-recovery-20260908` since; large artifacts belong on
  D:; the Intel box netboots the #63 recovery image from there and was
  kexec'd to #71 (RUNNING.md §7).
- **A campaign's oracle is part of its result.** Two suites that read
  78/78 regraded to 72/78 when the diagnostic oracle learned to reject a
  LAND error text it had missed; grade home.log/remote.log
  (`tests/run-diagnostics.py`) and the cleanup report, not guest.out alone.
