...
static inline struct own_ent *own_slot(uint64_t as, uint64_t page, int create)
{
	...
	page &= ~(uint64_t)0xfff;
	/* Hash the normalized page number. Byte addresses have twelve zero
	 * low bits: in this power-of-two table they used just 64 starting
	 * buckets per address space, putting long probe chains under own_lock.
	 * Normalize before hashing so every byte of a page names one entry. */
	h = as * 1099511628211ull + (page >> 12) * 14695981039346656037ull;
	for (i = 0; i < n; i++) {
