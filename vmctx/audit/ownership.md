...
The source owns native syscall entry, files, signals and process semantics.
The executor receives opaque context/MM/object identities, committed mapping
operations, CPU state and explicit page-transfer results; it must not infer
source behavior from its own OS syscall numbers, structures or `/proc` layout.
Every page has exactly one owner. Hashes, generations and traces diagnose a
contradiction; they cannot authorize replacement bytes or a new owner.

## Implemented foundations and their limits

- Retained native context descriptors fence task identity, monitor ownership,
  terminal queries and signals across PID reuse. An MM has its own retained
  identity and lifetime; task and address-space identity are different.
- Native BEGIN/QUERY/COMMIT tickets retain source entry policy and publish
  children before parent completion. Exit runs on the source while memory
  service is alive; terminal status comes from that retained source task.
- CPU negotiation and full architectural state cover fork, restore, signals
  and source exceptions. Runtime checkpoints charge actual guest execution to
  source CPU clocks. These controls do not prove every RCU/rseq/hotplug path.
- Executor context bindings retain distinct private and shared backing objects,
  change MM binding on exec and retain old views until operations drain.
  Mapping access combines source permissions with COW and ownership limits.
- Source custody reserves a transfer before capture, keeps replay bytes until
  exact installation acknowledgement, and retains failure state until native
  cancellation or abandonment. Capture cannot obstruct an incumbent fault's
  landing. Unconfirmed grants end the owned session; time and retry counts
  never confirm delivery.
- Local page reservations span pull, landing, acknowledgement and loan
  retirement. Release the reservation when a source claim needs a callback;
  no coherence/layout mutex may span network waits. Mapping replacement
  retires retained data, loans and old mappings before publishing construction.
- Shared identity is an opaque source object plus page offset, backed by one
  stable executor slot. Layout splits, protection and inheritance preserve it.
  This names bytes; it does not exclude native file, device or async writers.
- Process owners retain their group leader until group cancellation, reap
  adopted detached descendants through pidfds, and report completion only at
  ECHILD. SSH control EOF ends the owned session. Cleanup is part of the gate.

The remaining native MM representation is the central unfinished contract.
The following requirements supersede successful ordinary-suite observations.

## Native remote residency

Design under review, **not implemented or validated**. The measured failures
and evidence are in [the validation record](README.md).
